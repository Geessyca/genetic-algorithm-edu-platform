...
function geneticAlgorithm({ precisionRateValue, mutationRate, corDeParada, geracoesMax, populacaoInicial, repetirAte, metodoSelecao }) {
    ...
    let generation = 1;
    fitnessStatus.innerHTML = `Fitness rate achieved: <b>0%</b>/${precisionRateValue}%`;

    async function step() {
        population = population.sort((a, b) => calculateFitness(b.color, corDeParada) - calculateFitness(a.color, corDeParada));

        generationLabel.innerHTML = `<p>${generation}</p><span>Current generation</span>`;
        ...
        fitnessStatus.innerHTML = `Fitness rate achieved: <b>${fitnessPercent}%</b>/${precisionRateValue}%`;
