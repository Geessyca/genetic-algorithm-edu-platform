...
const LANG_DICT = {
    pt: {
        fitnessRate: "Taxa de aptidão alcançada: <b>{fitness}%</b>/{precision}%",
        bestFitness: "Melhor Aptidão",
        generationLabel: "<p>{generation}</p><span>Geração atual</span>",
        run: "Executar",
        simulation: "Simulação",
        statistic: "Estatística"
    },
    en: {
        fitnessRate: "Fitness rate achieved: <b>{fitness}%</b>/{precision}%",
        bestFitness: "Best Fitness",
        generationLabel: "<p>{generation}</p><span>Current generation</span>",
        run: "Run",
...
// Set language: 'pt' or 'en'
let currentLang = localStorage.getItem('lang') || (navigator.language ? navigator.language.slice(0,2) : 'pt');

/**
 * Returns translated string for a key and parameters.
 * @param {string} key
 * @param {Object} params
 * @returns {string}
 */
const t = (key, params = {}) => {
    let str = LANG_DICT[currentLang][key] || key;
...
/**
 * Calculates fitness based on color distance.
 * @param {number[]} individual
 * @param {number[]} targetColor
 * @returns {number}
 */
const calculateFitness = (individual, targetColor) => {
...
const fitnessChart = new Chart(ctx, {
    ...
    options: {
        responsive: true,
        scales: {
            x: { title: { display: true, text: currentLang === 'pt' ? "Geração" : "Generation" } },
            y: { title: { display: true, text: currentLang === 'pt' ? "Aptidão (%)" : "Fitness (%)" }, min: 0, max: 100 }
        }
    }
});

function atualizarIdiomaGrafico() {
    fitnessChart.data.datasets[0].label = t("bestFitness");
    fitnessChart.options.scales.x.title.text = currentLang === 'pt' ? "Geração" : "Generation";
    fitnessChart.options.scales.y.title.text = currentLang === 'pt' ? "Aptidão (%)" : "Fitness (%)";
    fitnessChart.update();
}

// Detect language change and update chart
window.addEventListener('storage', function(e) {
    if (e.key === 'lang') {
        currentLang = e.newValue || 'pt';
        atualizarIdiomaGrafico();
    }
});

// Also update chart on page load
atualizarIdiomaGrafico();
...
async function atualizarInterface(population, selectedIds, generation) {
    fishContainer.innerHTML = '<img id="bird" src="./assets/bird.gif"/>';
...
function geneticAlgorithm({ precisionRateValue, mutationRate, corDeParada, geracoesMax, populacaoInicial, repetirAte, metodoSelecao }) {
    ...
    let generation = 1;
    fitnessStatus.innerHTML = t("fitnessRate", { fitness: 0, precision: precisionRateValue });
    async function step() {
        population = population.sort((a, b) => calculateFitness(b.color, corDeParada) - calculateFitness(a.color, corDeParada));
        generationLabel.innerHTML = t("generationLabel", { generation });
        if (generation < 6) {
            animationReset();
        }
        ...
        fitnessStatus.innerHTML = t("fitnessRate", { fitness: fitnessPercent, precision: precisionRateValue });
